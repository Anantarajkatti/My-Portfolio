...
function AdminCourses() {
    ...
    const handleDelete=async(Course_id)=>{
     try{
      ...
      const response = await axios.delete(`http://localhost:5000/api/portfolio/delete-Course/${Course_id}`);
  ...
  return (
    <div>
      ...
      <div className="cardContainer grid grid-cols-3 tablet:grid-cols-1 tablet:px-5 gap-10  mt-10">
        {portfolioData.courseRes.map((Course) => (
          <div className="card shadow border py-5 ">
            <h1 className="bg-secondary px-5 py-3  font font-semibold">
              {" "}
              {Course.courseName}
            </h1>
            <h1 className="px-5 py-2 font-semibold">{Course.organization}</h1>
            <h1 className="px-5 py-2 ">{Course.description}</h1>
      ...
      <Modal
        ...
        title={selectItemForEdit ? "Edit Course" : " Add Course"}
        footer={null}
